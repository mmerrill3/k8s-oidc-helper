...
const oauthUrl = "https://accounts.google.com/o/oauth2/auth?redirect_uri=urn:ietf:wg:oauth:2.0:oob&response_type=code&client_id=%s&scope=openid+email+profile+groups&approval_prompt=force&access_type=offline"
...
func main() {
	...
	flag.String("cookieURL", "", "The URL where to exchange the cookie provided, for example, https://dashboard.k8s.ctnrva0.vonagenetworks.net/exchangeCookie")
	flag.String("cookie", "", "The value of the cookie generated by the oauth2-proxy using the web application flow of oauth2")
	flag.BoolP("web", "", false, "Generate auth using web flow")
	...
	var tokResponse = &helper.TokenResponse{}

	//fmt.Printf("Entered the web: %v\n", viper.GetBool("web"))

	if viper.GetBool("web") {
        helper.LaunchBrowserToCopyCookie(viper.GetBool("open"), oauthUrl, clientID)
        fmt.Print("Enter the auth json from exchangeToken page: ")
	    //helper.SetSTTY()
        ...
        tokResponse, err = helper.GetTokenFromAuth(auth)
        //fmt.Printf("json IDToken given is: %s\n", tokResponse.IdToken )

        if err != nil {
        	fmt.Printf("Error reading input%s\n", err)
            os.Exit(1)
        }
	} else if viper.GetString("cookie") != "" {
		var err error
		if viper.GetString("cookieURL") == "" {
			fmt.Printf("Must send in the cookieURL if a cookie is passed in\n")
			os.Exit(1)
		}
		tokResponse, err = helper.GetTokenFromCookie(viper.GetString("cookie"), viper.GetString("cookieURL"))
		if err != nil {
